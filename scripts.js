...
let customBgUrl = localStorage.getItem('customBg') || null;
let selectedBgUrl = localStorage.getItem('selectedBg') || null;
...
function switchBgTab(tab, btn) {
  ...
  BG_PRESETS[tab].forEach(item => {
    ...
    img.src = item.url;
    img.alt = item.label;
    img.className = 'bg-thumb' + (selectedBgUrl === item.url ? ' selected' : '');
...
function handleBgUpload(e) {
  ...
  reader.onload = ev => {
    ...
    selectedBgUrl = null;
    localStorage.setItem('customBg', customBgUrl);
    localStorage.removeItem('selectedBg');
    applyBgOverride(customBgUrl);
    closeBgModal();
  };
  reader.readAsDataURL(file);
}

function applyBgOverride(url) {
...
function updateBackground() {
  const override = customBgUrl || selectedBgUrl;
  if (override) { applyBgOverride(override); return; }
  const isDark = document.body.classList.contains('dark');
